...
public class StationeriesDAO implements DAO<StationeriesModel> {
	...
	public int Add(StationeriesModel bean) {
		String sql = "INSERT INTO STATIONERIES (GOODS_ID,GOODS_NAME,DESCRIPT,COLORL) "
				+ "VALUES (GOODS_ID_SEQ.NEXTVAL,'" + bean.getGoods_name() + "','" + bean.getDescript() + "','"
				+ bean.getColor() + "')";
		return db.add(sql);
	}
	...
	public int Update(StationeriesModel bean) {
		String sql = "UPDATE STATIONERIES SET GOODS_NAME = '" + bean.getGoods_name() + "',DESCRIPT = '"
				+ bean.getDescript() + "',COLORL='" + bean.getColor() + "' WHERE GOODS_ID ='" + bean.getGoods_id()
				+ "'";
